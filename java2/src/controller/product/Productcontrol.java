...
public class Productcontrol implements Initializable {
    ...
    void show(String search) {
    	if(vbox.getChildren().isEmpty() == false) { // isEmpty() : 해당 객체내 비어 있는지 확인
    			// vbox내 객체가 비어있지 않으면
    		vbox.getChildren().remove(0); // vbox내 기존 객체를 삭제;
    	}
    	// 1. 모든 제품 가져오기
    			ArrayList< Product > productlist = ProductDao.produtctDao.list(Home.category, search);
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		show(null); // 화면을 처음열렸을 때는 검색어를 null을 넣어줌
	}
}
