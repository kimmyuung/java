...
public static void main(String[] args) {
	...
	if(select == 1) {
		...
		콜라구매수++;
		콜라재고수--;
		if(콜라재고수 == 0) {System.out.println("재고가 소진되었습니다. 재입고를 기다려주세요");}
	}
	else if(select == 2) {
		System.out.println("환타를 구매합니다.");
		환타구매수++;
		환타재고수--;
		if(환타재고수 == 0) {System.out.println("재고가 소진되었습니다. 재입고를 기다려주세요");}
	}
	else if(select == 3) {
		System.out.println("사이다를 구매합니다.");
		사이다구매수++;
		사이다재고수--;
		if(사이다재고수 == 0) {System.out.println("재고가 소진되었습니다. 재입고를 기다려주세요");}
	}
	else if(select == 4) {
		...
		if(select1 == 1) {
			...
			else {System.out.println("결제액이 부족합니다 " + (총결제액-결제액) + "원 만큼 더 투입해주세요");}
		}
		else if(select1 == 2) {
