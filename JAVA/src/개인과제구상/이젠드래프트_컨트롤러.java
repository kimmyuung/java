...
public class 이젠드래프트_컨트롤러 {
	...
	public static String[] 팀원시너지 = {"야자조","구1조","구2조","구3조","구4조","구5조","구6조", "동갑내기","혼자가편해요"};
	...
	void 팀원저장() {
		try {
			...
			for(팀원 temp : 팀원리스트) {
				String 총내용 = temp.get팀원번호() + "," + temp.get팀원이름() + "," + temp.get팀원특성() + "," 
						+ temp.get팀원시너지() + "\n";
				fileOutputStream.write(총내용.getBytes());
			}
			}catch(Exception e) {System.out.println("팀원 저장 오류 발생!" + e);}
	}
	void 팀원로드() {
		try {
		...
		} catch(Exception e) {System.out.println("팀원 불러오기 오류 발생!" + e);}
	}
	void 효과판단() {}
	void 시너지판단() {}
	void 게임점수save() {
		...
		for(팀원 temp : 팀원리스트) {
			String 총내용 = "게임점수"; // 게임점수 저장하면 될듯
			fileOutputStream.write(총내용.getBytes());
		}
		}catch(Exception e) {System.out.println("점수 저장 오류 발생!" + e);}
	}
	void 게임점수load() {}
	boolean 팀원편집(String name, String skill, String combie) {
		for(팀원 temp : 팀원리스트) {
			if(!(temp.get팀원이름().equals(name) ) ) {
				return false;
			} else {temp.set팀원이름(name);
			temp.set팀원시너지(combie);
			temp.set팀원특성(skill); return true;
			}
		}
		return false;
	}
