...
public class Day18_1 {
	...
	public static void main(String[] args) {
		// ArrayList 클래스 : 배열과 유사 하지만 길이(저장수)가 가변[자동]
		...
		// 2. 리스트객체 내 요소 추가 : 리스트객체명.add(객체명) : 해당 객체를 리스트객체 내 추가[비어있는 마지막 인덱스]
		// * : 리스트객체명.add(인덱스, 객체명) : 해당 인덱스에 객체를 리스트객체내 삽입 [ 기존 인덱스는 인덱스 증가 ]
		// * : 리스트객체명.get(인덱스) : 리스트객체내 해당 인덱스 객체 호출
		// * : 리스트객체명.remove(인덱스) : 리스트객체내 해당 인덱스 객체 삭제 [ 삭제된 인덱스 뒤로 인덱스-1 ]
		// * : 리스트객체명.remove(객체명) : 리스트객체내 해당 객체 삭제
		// * : 리스트객체명.clear() : 리스트 객체내 모든 객체 비우기
		// * : 리스트객체명.size() : 리스트객체내 객체수 [ 길이 ]
		// * : 리스트객체명.isEmpty() : 리스트객체내 비어있는지 확인 [ 리스트 객체내 비어있는지 확인 ]
			// 리스트객체내 객체 1개도 없으면 -> true
			// 리스트객체내 1개 이상이면 -> false
		// * : 리스트객체명.contains(객체명) : 리스트객체내 해당 객체 존재 여부 확인( true/ false)
		// * : 리스트객체명.indexOf(객체명) : 리스트객체내 해당 객체의 인덱스번호
