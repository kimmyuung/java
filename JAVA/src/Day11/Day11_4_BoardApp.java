			...
			try { // 예외발생 : 사용자가 문자입력시 예외발생!!! -> catch 이동 
				...
				if( ch == 1 ) {
					...
					if( ch2 == 1 ) { }
						...
						System.out.print("현 게시물 패스워드 : "); String pw = scanner.next();
						System.out.println("변경 제목 : ");	   String title = scanner.nextLine();
						System.out.println("변경 내용 : ");	   String context = scanner.nextLine();
						boolean result = Controller.update(index, pw, title, context); // 수정할 인덱스번호,패스워드,수정할제목,수정할내용 =>  인수 
						if(result) System.out.println("수정 성공");
						else System.out.println("수정 실패 [[ 비밀번호 오류 ]]");
					}
					else if( ch2 == 3 ) { 
